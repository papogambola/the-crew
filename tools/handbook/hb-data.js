...
const ATTR_WHAT={
  muscle:"Carrying, forcing, holding a door, and being the reason somebody reconsiders. The Enforcer's attribute.",
  brains:"Planning, numbers, locks, reading a room's paperwork. The Safecracker's, the Forger's, the Lookout's and the Launderer's.",
  tech:"Networks, cameras, charges, anything with a circuit in it. The Hacker's and the Demolitions man's.",
  charm:"Talking your way in, selling a lie, being the name in the room. The Grifter's, the Fixer's, the Pickpocket's and the Face's.",
  nerve:"Holding still when it goes wrong, driving, walking in as though invited. The Wheelman's, the Infiltrator's, the Overwatch's, the Cleaner's and the Smuggler's.",
};
exports.attrs=()=>T(["","Attribute","What it is for"],
  D.ATTRS.map(a=>['<b class="mono">'+D.ATTR_ABBR[a]+'</b>',esc(a[0].toUpperCase()+a.slice(1)),esc(ATTR_WHAT[a])]));

exports.techs=()=>T(["Trade","Leans on","Comes with","What they do","On the map"],
  D.TECHS.map(t=>['<b>'+esc(t.l)+'</b>','<span class="mono">'+D.ATTR_ABBR[t.a]+'</span>',esc(t.know),esc(t.d),
    '<span class="mono">'+esc(({exit:"at the car",watch:"on the corner",door:"at the way in",inside:"in the room"})[D.FM_TECH_GLYPH&&({wheelman:"exit",smuggler:"exit",cleaner:"exit",lookout:"watch",overwatch:"watch",hacker:"watch",launderer:"watch",fixer:"watch",forger:"door",grifter:"door",face:"door",infiltrator:"door",pickpocket:"door",safecracker:"inside",demolitions:"inside",enforcer:"inside"})[t.k]]||"")+'</span>']));
...
exports.cats=()=>T(["Kind of job","What it leans on"],
  D.CATS.map(c=>['<b>'+esc(c.l)+'</b>',Object.keys(c.w).map(a=>'<span class="mono">'+D.ATTR_ABBR[a]+'</span>×'+c.w[a]).join("  ")
